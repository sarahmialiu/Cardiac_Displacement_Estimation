...
def deform_mask_jump_pairs(mask, hzn_flow, vert_flow, dep_flow):
    ...
    # --- Figure setup ---
    fig, ((real_ax, pred_ax), (res_ax, vector_ax)) = plt.subplots(2, 2)
    ...
    pred_ax.set_title(f"Deformed Mask (Deformed from Real Mask Frame 0)")
    ...
    vector_img  = vector_ax.imshow(mask[0, :, :, slice_index], cmap="gray")
    ...
    def update_display():
        ...
        if orientation == "X, Y":
            ...
            vector_data = mask[0, : , slice_idx, :].T
        ...
        vector_img.set_data(vector_data)
        quiver.set_UVC(5*hzn_flow_data, 5*vert_flow_data)

        vector_ax.set_title(f"Displacement \nFrame 0 to {time_idx+1}")
...
# Load volume
mask = np.load("/home/sarahl/Documents/Fall Rotation/VoxelMorph/out/Masked/resizedmask_jump.npy", allow_pickle=True)
hzn_flow = np.load("/home/sarahl/Documents/Fall Rotation/VoxelMorph/out/Masked/hzn_flow_jump.npy")
vert_flow = np.load("/home/sarahl/Documents/Fall Rotation/VoxelMorph/out/Masked/vert_flow_jump.npy")
dep_flow = np.load("/home/sarahl/Documents/Fall Rotation/VoxelMorph/out/Masked/dep_flow_jump.npy")
deform_mask_jump_pairs(mask, hzn_flow, vert_flow, dep_flow)