...
def plot_history(hist, dir):
    ...
    plt.savefig(dir + '/training_loss.png')
...
weights = [4000, 1000000]                                     # weights to apply to loss (NCC or MSE) and deformation loss (l2 Grad)
...
losses = [losses.MSE().loss, losses.Grad('l2', loss_mult=2).loss] #[losses.NCC(win=(12,12)).loss, losses.Grad('l2', loss_mult=2).loss] # loss_mult should be same as int_downsize
min_val_loss = 100
...
for epoch in range(initial_epoch, epochs):
    ...
    train_losses.append(np.mean(epoch_total_loss))
    ...
    val_losses.append(np.mean(val_epoch_total_loss))
